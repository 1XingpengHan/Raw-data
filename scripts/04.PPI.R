...
for(i in 2:length(file_path)){
  ...
  TF_chipbase <- rbind(TF_chipbase,TF,fill=TRUE)
}
#
sum <- apply(TF_chipbase[,4:5],1,sum)
table(sum)
mRNA_TF <- TF_chipbase[which(sum>5),c(8,1)]
...
write.csv(table_TF,"TableS2 mRNA-TF.csv",row.names = F,quote = F)
...
# # miRNA_Hs <- miRNA_Hs[miRNA_Hs$geneName %in% genes$genes,]
# # miRNA_Hs <- miRNA_Hs[miRNA_Hs$direct_indirect == "DIRECT",]
# # mRNA_miRNA <- miRNA_Hs[which(is.na(miRNA_Hs$condition)),c(2,3)]
# mRNA_miRNA <- miRNA_Hs[miRNA_Hs$geneName %in% genes$genes,]
# mRNA_miRNA <- mRNA_miRNA[,c(2,3)]
# colnames(mRNA_miRNA)[1] <- "node1"
# colnames(mRNA_miRNA)[2] <- "node2"
# write.csv(mRNA_miRNA,"7-mRNA-miRNA_nodes.csv",row.names = F,quote = F)
# miRNA_attr <- data.frame("node" = c(unique(mRNA_miRNA$node1),unique(mRNA_miRNA$node2)),
#                         "attribute" = c(rep("mRNA",length(unique(mRNA_miRNA$node1))),
#                                         rep("miRNA",length(unique(mRNA_miRNA$node2)))))
# write.csv(miRNA_attr,"7-mRNA-miRNA_attribute.csv",row.names = F,quote = F)
...
mRNA_RBP <- RBP_Starbase[which(RBP_Starbase$clusterNum>24),c(3,1)]
...
mRNA_Drug <- Drug_CTD[which(Drug_CTD$`Reference Count`>3),c(10,1)]
